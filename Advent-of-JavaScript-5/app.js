...
for (let i = 0 ; i < episodes.length ; i++) {
  ...
  li.innerHTML = `
    <label for="episode-${episodes[i].id}">
      <input type="checkbox" name="episode-${episodes[i].id}" id="episode-${episodes[i].id}" />
...
document.addEventListener('click', function(event) {
  if (event.target.type === 'checkbox') {
    if (event.shiftKey && lastChecked) {
      ...
      checkboxes.slice(Math.min(start,end), Math.max(start,end) + 1).forEach(checkbox => checkbox.checked = true);
    }
    lastChecked = event.shiftKey ? lastChecked : event.target;
  }
});