...
function App() {
  ...
  const selectedEpisode = episodes.find((episode) => episode.id === selected);
  ...
  return (
    <div className="wrapper">
        ...
        <ul id="tabs">
          {episodes.map((episode) => (
            <li
              ...
              key={episode.id}
            >
              <a href="#" onClick={() => setSelected(episode.id)}>
      ...
      <main>
        <div key={selected} className="cover">
          <img src={`./${selectedEpisode.cover}`} alt={selectedEpisode.title} />
        </div>
        <div className="content">
          <h1>{selectedEpisode.title}</h1>
          <p>{selectedEpisode.description}</p>
          <a href={selectedEpisode.link} className="more">
            More
          </a>
        </div>
      </main>
